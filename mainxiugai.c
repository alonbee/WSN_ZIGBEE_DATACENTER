...
void main(void)
{
  ...
  GPIO_Init( ADDR_HIGH_PORT2,ADDR_HIGH_PIN2,GPIO_Mode_Out_PP_Low_Slow);
  ...
  //Config=DS18B20_Read();
  s_connectionreset();
  Temprature=s_read_byte();
  Temprature=Temprature | (s_read_byte()<<8);
  Check_sum=s_read_byte();
...
*/
//sth11
  u8 s_write_byte(unsigned char value)
//----------------------------------------------------------------------------------
// writes a byte on the Sensibus and checks the acknowledge
{
  ...
  u8 s_read_byte(unsigned char ack)
//----------------------------------------------------------------------------------
...
u8 s_measure(unsigned char *p_value, unsigned char *p_checksum, unsigned char mode)
//----------------------------------------------------------------------------------
